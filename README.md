...
6. For local development you should set `accounts.api_jwt_token` to token generated by [InNoHassle Accounts API](https://api.innohassle.ru/accounts/v0/tokens/generate-service-token?sub=accounts-local-dev&scopes=users&only_for_me=true) and saved in `settings.yaml`, then get your user id from [InNoHassle Accounts API](https://api.innohassle.ru/accounts/v0/docs#/Users/users_get_me) and use `innohassle-local-dev/login` endpoint to login under that user id.
